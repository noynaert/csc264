...
Floating point operations have gone through a series of changes and improvements over the years. They use entirely different methods of processing floating point numbers. If you get involved in an open source project or you end up trying to understand some bit of virus or malware code, you may run into some code that uses older techniques. In this section we will look at some of the older floating point techniques that have been used .
## Software Math vs Hardware Math

Natively, CPUs only do integer math, and they only do addition, subtraction, and bit manipulation.  The do not handle floating point arithmetic at all, and they do not do multiplication or division.

Now most processors have hardware dedicated to multiplication, division, and floating point arithmetic.  They are called FPUs
...
Another approach was to avoid doing floating point arithmetic altogether. For example, financial calculations would be done representing pennies as integers rather than dollars as floating point numbers. This avoided the pitfalls of floating point arithmetic, such as rounding errors, but required careful handling of scaling and formatting.
...
![8086 and 8087 processors](images/8086_8087.jpg)
